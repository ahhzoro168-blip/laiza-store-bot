...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_allowed(user_id):
        keyboard = [[KeyboardButton("🛍 Shop")], [KeyboardButton("⚙️ Manage")]]
    else:
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== DELETE CATEGORY =====
        ...
        conn.commit()
        await query.message.reply_text("🗑 Category Deleted")

    # ===== STOCK =====
    elif data.startswith("stockcat_page_"):
        page = int(data.split("_")[2])
        cursor.execute("SELECT * FROM categories")
        await query.message.edit_reply_markup(reply_markup=build_grid(cursor.fetchall(), "stockcat", page))
        ...
        for pid, file_id, price, _ in cursor.fetchall():
            ...
            keyboard.append([InlineKeyboardButton("🗑 Delete Product", callback_data=f"deleteproduct_{pid}")])
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "add_cat":
        ...
        cursor.execute("INSERT INTO products (file_id, price, category_id) VALUES (?,?,?)",
        ...
        keyboard = [[KeyboardButton("➕ Add")], [KeyboardButton("📦 Stock")], [KeyboardButton("🔙 Back")]]
